...
const EditContent = ({ post, tags }) => {
  ...
  return (
    <div className={styles.editContainer}>
      <Link href={"/"}>
        <a className={styles.backBtn}>بازگشت به مرحله قبل</a>
      </Link>
      <h2>ویرایش محتوا</h2>
        ...
        <div className={styles.formGroup}>
          <div className={styles.selectContainer}>
            <select
              ...
              id=""
              onChange={(e) => {
                onSubjectSelect(e.target.value);
              }}
            >
              {tags.map((item, index) => {
                return (
          ...
          <div className={styles.subjectsContainer}>
            {subjects?.map((item, index) => {
              return (
                <div
                  onClick={() => onSubjectSelect(item)}
                  key={index}
                  className={styles.subject}
                >
                  {tags.find((p) => p._id === item).title}
                </div>
              );
            })}
          </div>
      ...
      <button className={styles.submitBtn} onClick={() => handleEditPost()}>
        تایید نهایی
      </button>
    </div>
  );
